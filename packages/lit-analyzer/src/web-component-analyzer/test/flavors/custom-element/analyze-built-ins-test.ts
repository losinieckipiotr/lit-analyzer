...
import {
  getCurrentTsModule,
  getCurrentTsModuleDirectory,
  tsTest
} from "../../helpers/ts-test.js";
...
tsTest("analyzeSourceFile on lib.dom.ts returns correct result", t => {
  ...
  const program = tsModule.createProgram(
    [join(getCurrentTsModuleDirectory(), "lib.dom.d.ts")],
    {}
  );
  ...
  if (domLibSourceFile == null) {
    throw new Error(
      `Couldn't find '${endsWithLibDom}'. Have you included the 'dom' lib in your tsconfig?`
    );
  }
  ...
  const scriptDefinition = result.componentDefinitions?.find(
    d => d.tagName === "script"
  );
  t.truthy(scriptDefinition);

  const srcProperty = scriptDefinition!.declaration?.members.find(
    m => m.kind === "property" && m.propName === "src"
  );

  t.truthy(srcProperty);
  t.true(
    srcProperty!.visibility === undefined ||
      srcProperty!.visibility === "public",

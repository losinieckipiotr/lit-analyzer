...
tsTest(
  "Discovers only one element defined using multiple customElements.define",
  t => {
    const {
      results: [result]
    } = analyzeTextWithCurrentTsModule(`
...
tsTest(
  "Discovers elements defined using customElements.define without string literal",
  t => {
    const {
      results: [result]
    } = analyzeTextWithCurrentTsModule(`
...
tsTest(
  "Doesn't crash when encountering component declaration nodes that can't be resolved",
  t => {
    const {
      results: [result]
    } = analyzeTextWithCurrentTsModule(`
...
tsTest("Correctly discovers multiple declarations", t => {
  ...
  t.is(componentDefinitions[0].tagName, "my-element");
  t.is(
    componentDefinitions[0].declaration?.members?.some(
      m => m.propName === "prototype"
    ),
    false
  );
  t.is(

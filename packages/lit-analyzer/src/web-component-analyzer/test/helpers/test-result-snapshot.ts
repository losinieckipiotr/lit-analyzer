...
function testResult(
  ...
  test(testName, async t => {
    ...
    const nonEmptyResults = results.filter(
      result => result.componentDefinitions.length > 0
    );
    ...
    const sortedResults = nonEmptyResults
      ...
      .map(result => ({
        ...result,
        componentDefinitions: result.componentDefinitions.sort((a, b) =>
...
export function testResultSnapshot(globs: string[]): void {
  testResult(
    `Snapshot Test: ${globs.map(glob => `"${glob}"`).join(", ")}`,
    globs,
    (_results, _program, t) => {
      // const declarations = arrayFlat(
      // 	results.map(result =>
      // 		result.componentDefinitions.map(def => def.declaration)
      // 	)
      // );
      // const summary = {
      // 	elements: results.reduce(
      // 		(acc, result) => acc + result.componentDefinitions.length,
      // 		0
      // 	),
      // 	tagNames: arrayFlat(results.map(result => result.componentDefinitions))
      // 		.map(def => def.tagName)
      // 		.join(", "),
      // 	members: declarations.reduce(
      // 		(acc, decl) => acc + decl!.members.length,
      // 		0
      // 	),
      // 	cssParts: declarations.reduce(
      // 		(acc, decl) => acc + decl!.cssParts.length,
      // 		0
      // 	),
      // 	cssProps: declarations.reduce(
      // 		(acc, decl) => acc + decl!.cssProperties.length,
      // 		0
      // 	),
      // 	events: declarations.reduce(
      // 		(acc, decl) => acc + decl!.events.length,
      // 		0
      // 	),
      // 	slots: declarations.reduce((acc, decl) => acc + decl!.slots.length, 0),
      // 	methods: declarations.reduce(
      // 		(acc, decl) => acc + decl!.methods.length,
      // 		0
      // 	),
      // 	mixins: declarations
      // 		.map(

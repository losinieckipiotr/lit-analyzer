 ...
 * @param config
 */
export const analyzeCliCommand: CliCommand = async (
...
`,
  ...
  const context: AnalyzeGlobsContext = {
    ...
    willAnalyzeFiles(filePaths: string[]): void {
      log(
        `Web Component Analyzer analyzing ${filePaths.length} file${filePaths.length === 1 ? "" : "s"}...`,
        config
      );
    },
    emitAnalyzedFile(file, result, { program }): Promise<void> | void {
      ...
      if (outStrategy === "console_stream") {
        if (result.componentDefinitions.length > 0) {
          ...
          console.log(
            transformResults(result, program, {
              ...config,
  ...
  if (outStrategy === "console_bulk") {
    ...
    /* eslint-disable-next-line no-console */
    console.log(
      transformResults(filteredResults, program, {
        ...config,
        cwd: config.cwd || process.cwd()
      })
    );
    ...
    const outputResultMap = await distributeResultsIntoFiles(
      filteredResults,
      config
    );
    ...
    for (const [outputPath, results] of outputResultMap) {
      if (outputPath != null) {
        if (config.dry) {
          const tagNames = arrayFlat(
            results.map(result =>
              result.componentDefinitions.map(d => d.tagName)
            )
          );
          log(
            `[dry] Intending to write ${tagNames} to ./${relative(process.cwd(), outputPath)}`,
            config
          );
        } else {
          const content = transformResults(results, program, {
            ...config,
            cwd: config.cwd || dirname(outputPath)
          });
...
export async function analyzeAndTransformGlobs(
  ...
  const { results, program } = await analyzeGlobs(
    Array.isArray(inputGlobs) ? inputGlobs : [inputGlobs],
    config
  );
    ...
    for (const result of results) {
      ...
      const filename = relative(
        ...
        basename(
          ...
          extname(result.sourceFile.fileName)
        )
      );

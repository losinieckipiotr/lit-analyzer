...
import {
  ...
  resolveType as resolveTypeUnsafe,
  SimpleType,
...
type ResultCacheVal = WeakMap<SimpleType, WeakMap<SimpleType, boolean>>;

const DEFAULT_RESULT_CACHE = new Map<string, ResultCacheVal>();
...
function isCacheableType(
  ...
  switch (simpleType.kind) {
    ...
    case "INTERSECTION":
      if (
        options.genericParameterMapA.size !== 0 ||
        options.genericParameterMapB.size !== 0
      ) {
        return false;
      }
  ...
  return (
    !("typeParameters" in simpleType) &&
    !["GENERIC_ARGUMENTS", "GENERIC_PARAMETER", "PROMISE", "LAZY"].includes(
      simpleType.kind
    )
  );
}

function getTupleLengthType(tuple: SimpleTypeTuple): SimpleType {
...
function isAssignableToSimpleTypeInternal(
  ...
  if (
    options.depth >= options.config.maxDepth! ||
    options.operations.value >= options.config.maxOps!
  ) {
    options.preventCaching();
    return true;
  }
    ...
    if (options.comparingTypes.get(typeA)!.has(typeB)) {
      ...
      if (options.config.debug) {
        logDebug(
          options,
          "comparing types",
          "Returns true because this relation is already being checking"
        );
      }
  ...
  switch (typeB.kind) {
    ...
    case "UNION": {
      ...
      const types = typeB.types.filter(
        t =>
          resolveType(t, options.genericParameterMapB) !==
          DEFAULT_GENERIC_PARAMETER_TYPE
      );
    ...
    case "INTERSECTION": {
      ...
      const combined = reduceIntersectionIfPossible(
        typeB,
        options.genericParameterMapB
      );
      ...
      if (combined.kind === "NEVER") {
        if (options.config.debug) {
          logDebug(
            options,
            "intersection",
            `Combining types in intersection is impossible. Comparing with 'never' instead.`
          );
        }
      ...
      if (options.config.debug) {
        if (combined !== typeB) {
          logDebug(
            options,
            "intersection",
            `Types in intersection were combined into: ${simpleTypeToStringLazy(combined)}`
          );
        }
      }
    ...
    case "GENERIC_ARGUMENTS": {
      const updatedGenericParameterMapB = extendTypeParameterMap(
        typeB,
        options.genericParameterMapB
      );
      ...
      if (options.config.debug) {
        logDebug(
          ...
          Array.from(updatedGenericParameterMapB.entries())
            ...
            .join("; "),
          "typeParameters" in typeB.target
            ? ""
            : "[No type parameters in target!]"
        );
      }
    ...
    case "GENERIC_PARAMETER": {
      ...
      const realTypeB =
        resolvedArgument || typeB.default || DEFAULT_GENERIC_PARAMETER_TYPE;
  ...
  switch (typeA.kind) {
    ...
    case "GENERIC_PARAMETER": {
      ...
      const realTypeA =
        resolvedArgument || typeA.default || DEFAULT_GENERIC_PARAMETER_TYPE;
    ...
    // [typeA] (expand)
    case "GENERIC_ARGUMENTS": {
      const updatedGenericParameterMapA = extendTypeParameterMap(
        typeA,
        options.genericParameterMapA
      );
      ...
      if (options.config.debug) {
        logDebug(
          ...
          Array.from(updatedGenericParameterMapA.entries())
            .map(([name, type]) => `${name}=${simpleTypeToStringLazy(type)}`)
            .join("; "),
          "typeParameters" in typeA.target
            ? ""
            : "[No type parameters in target!]"
        );
      }
    ...
    case "UNION": {
      ...
      const types = typeA.types.filter(
        t =>
          resolveType(t, options.genericParameterMapA) !==
            DEFAULT_GENERIC_PARAMETER_TYPE ||
  ...
  switch (typeA.kind) {
    ...
    case "NON_PRIMITIVE": {
      if (options.config.debug) {
        logDebug(
          options,
          "object",
          `Checking if typeB is non-primitive [primitive=${isSimpleTypePrimitive(typeB)}] [hasName=${typeB.name != null}]`
        );
      }
    ...
    case "ENUM_MEMBER": {
      ...
      if (
        resolveType(typeA.type, options.genericParameterMapA).kind ===
          "NUMBER_LITERAL" &&
        ["NUMBER", "NUMBER_LITERAL"].includes(typeB.kind)
      ) {
        if (typeB.name != null) {
          return false;
        }
    ...
    case "METHOD": {
      ...
      if (options.config.debug) {
        logDebug(
          options,
          "function",
          `Checking if return type of typeA is 'void'`
        );
      }

      if (
        !isAssignableToSimpleTypeKind(typeA.returnType, SimpleTypeKind.VOID)
      ) {
        ...
        if (options.config.debug) {
          logDebug(
            options,
            "function",
            `Return type is not void. Checking return types`
          );
        }
      ...
      const paramTypesA =
        typeAThisParam == null
          ? typeA.parameters
          : typeA.parameters.filter(arg => arg !== typeAThisParam);
      const paramTypesB =
        typeBThisParam == null
          ? typeB.parameters
          : typeB.parameters.filter(arg => arg !== typeBThisParam);
      ...
      const requiredParamCountB = paramTypesB.reduce(
        (sum, param) => (param.optional || param.rest ? sum : sum + 1),
        0
      );
      if (requiredParamCountB > paramTypesA.length) {
        if (options.config.debug) {
          logDebug(
            options,
            "function",
            `typeB has more required params than typeA: ${requiredParamCountB} > ${paramTypesA.length}`
          );
        }
        return false;
      }
      ...
      for (
        let i = 0;
        i < Math.max(paramTypesA.length, paramTypesB.length);
        i++
      ) {
        ...
        if (paramA == null && prevParamA?.rest) {
          if (options.config.debug) {
            logDebug(
              options,
              "function",
              `paramA is null and but last param in typeA is rest. Use that one.`
            );
          }
        ...
        if (paramB == null && prevParamB?.rest) {
          if (options.config.debug) {
            logDebug(
              options,
              "function",
              `paramB is null and but last param in typeB is rest. Use that one.`
            );
          }
        ...
        if (paramA == null) {
          if (paramB != null && !paramB.optional && !paramB.rest) {
            if (options.config.debug) {
              logDebug(
                options,
                "function",
                `paramA is null and paramB is null, optional or has rest`
              );
            }
            return false;
          }
          ...
          if (options.config.debug) {
            logDebug(
              options,
              "function",
              `paramA is null and paramB it not null, but is optional or has rest`
            );
          }
        ...
        const resolvedTypeA = resolveType(
          paramA.type,
          options.genericParameterMapA
        );
        const resolvedTypeB = resolveType(
          paramB.type,
          options.genericParameterMapB
        );
        ...
        const paramAType =
          paramA.rest && resolvedTypeA.kind === "ARRAY"
            ? resolvedTypeA.type
            : paramA.type;
        const paramBType =
          paramB.rest && resolvedTypeB.kind === "ARRAY"
            ? resolvedTypeB.type
            : paramB.type;
        if (paramA.rest) {
          if (options.config.debug) {
            logDebug(
              options,
              "function",
              `paramA is 'rest' and has been resolved to '${simpleTypeToStringLazy(paramAType)}'`
            );
          }
        }
        ...
        if (paramB.rest) {
          if (options.config.debug) {
            logDebug(
              options,
              "function",
              `paramB is 'rest' and has been resolved to '${simpleTypeToStringLazy(paramBType)}'`
            );
          }
        }
    ...
    case "CLASS": {
      ...
      const typeAHasZeroMembers = isObjectEmpty(typeA, {
        ignoreOptionalMembers: (
          ["UNKNOWN", "NON_PRIMITIVE"] as SimpleTypeKind[]
        ).includes(typeB.kind)
      });
      ...
      if (
        typeAHasZeroMembers &&
        typeA.call == null &&
        (typeA.ctor == null || typeA.kind === "CLASS")
      ) {
        if (options.config.debug) {
      ...
      switch (typeB.kind) {
        ...
        case "CLASS": {
          ...
          const requiredMembersInTypeAExistsInTypeB = and(membersA, memberA => {
            ...
            const memberB = membersB.find(
              memberB => memberA.name === memberB.name
            );
          ...
          if (!requiredMembersInTypeAExistsInTypeB) {
            if (options.config.debug) {
              logDebug(
                options,
                "object-type",
                `Didn't find required members from typeA in typeB`
              );
            }
            return false;
          }
          ...
          if (typeA.ctor != null && typeA.kind !== "CLASS") {
            if (options.config.debug) {
              logDebug(
                options,
                "object-type",
                `Checking if typeB.ctor is assignable to typeA.ctor`
              );
            }
            ...
            if (typeB.ctor != null && typeB.kind !== "CLASS") {
              ...
              if (options.config.debug) {
                logDebug(
                  options,
                  "object-type",
                  `Expected typeB.ctor to have a ctor`
                );
              }
              return false;
            }
          ...
          if (typeA.call != null) {
            if (options.config.debug) {
              logDebug(
                options,
                "object-type",
                `Checking if typeB.call is assignable to typeA.call`
              );
            }
          ...
          const typeBIsEmpty =
            membersB.length === 0 &&
            typeB.call == null &&
            ((typeB.kind !== "CLASS" && typeB.ctor == null) ||
              typeB.kind === "CLASS");
          if (membersInCommon === 0 && !typeBIsEmpty) {
            if (options.config.debug) {
              logDebug(
                options,
                "object-type",
                `typeB has 0 members in common with typeA and there are more than 0 members in typeB`
              );
            }
          ...
          const membersInTypeBAreAssignableToMembersInTypeA = and(
            membersB,
            memberB => {
              const memberA = membersA.find(
                memberA => memberA.name === memberB.name
              );
              if (memberA == null) {
                return true;
              }
              if (options.config.debug) {
                logDebug(
                  options,
                  "object-type",
                  `Checking member '${memberA.name}' types`
                );
              }
              return isAssignableToSimpleTypeCached(
                memberA.type,
          ...
          if (options.config.debug) {
            if (!membersInTypeBAreAssignableToMembersInTypeA) {
              logDebug(
                options,
                "object-type",
                `Not all members in typeB is assignable to corresponding members in typeA`
              );
            } else {
              logDebug(
                options,
                "object-type",
                `All members were checked successfully`
              );
            }
          }
...
function reduceIntersectionIfPossible(
  simpleType: SimpleTypeIntersection,
  ...
  const NEVER_TYPE: SimpleTypeNever = { kind: SimpleTypeKind.NEVER };

  for (const member of simpleType.types) {
    const resolvedType = resolveType(member, parameterMap);
    typeKindMap.set(resolvedType.kind, [
      ...(typeKindMap.get(resolvedType.kind) || []),
      resolvedType
    ]);
    ...
    if (isSimpleTypePrimitive(resolvedType)) {
      if (isSimpleTypeLiteral(resolvedType)) {
        if (
          primitiveLiteralSet.has(resolvedType.kind) &&
          primitiveLiteralSet.get(resolvedType.kind) !== resolvedType.value
        ) {
          return NEVER_TYPE;
        }
  ...
  if ((typeKindMap.get(SimpleTypeKind.TUPLE)?.length || 0) > 1) {
    let len: number | undefined = undefined;
    for (const type of typeKindMap.get(
  ...
  if (
    typeKindMap.size === 1 &&
    (typeKindMap.get(SimpleTypeKind.OBJECT)?.length || 0) > 1
  ) {
          ...
          const combinedMemberType = reduceIntersectionIfPossible(
            {
              ...
              types: [members.get(member.name)!.type, member.type]
            },
            parameterMap
          );
          if (combinedMemberType.kind === SimpleTypeKind.NEVER) {
    ...
    return {
      ...(typeKindMap.get(SimpleTypeKind.OBJECT)![0] as SimpleTypeObject),
...
function isObjectEmpty(
  simpleType: SimpleTypeObjectTypeBase,
  { ignoreOptionalMembers }: { ignoreOptionalMembers?: boolean }
): boolean {
  return (
    simpleType.members == null ||
    simpleType.members.length === 0 ||
    (ignoreOptionalMembers && !simpleType.members.some(m => !m.optional)) ||
    false
  );
}

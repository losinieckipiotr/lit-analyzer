...
export function getJsDoc(
  ...
  return {
    description:
      jsDocNode.comment == null
        ? undefined
        : unescapeJSDoc(String(jsDocNode.comment)),
    node: jsDocNode,
    tags:
      jsDocNode.tags == null
        ? []
        : arrayDefined(
            jsDocNode.tags.map(node => {
              ...
              const typeExpressionPart =
                "typeExpression" in node
                  ? (node as JSDocTypeTag).typeExpression?.getText()
                  : undefined;
              const namePart =
                "name" in node
                  ? (node as JSDocParameterTag).name?.getText()
                  : undefined;
...
export function parseSimpleJsDocTypeExpression(
  ...
  switch (str.toLowerCase()) {
    case "undefined":
      return SIMPLE_TYPES.UNDEFINED;
    case "null":
      return SIMPLE_TYPES.NULL;
    case "string":
  ...
  if (genericArgsMatch != null) {
    ...
    return {
      ...
      typeArguments: typeArgStrings.map(typeArg =>
        parseSimpleJsDocTypeExpression(typeArg, context)
      )
    };
  }

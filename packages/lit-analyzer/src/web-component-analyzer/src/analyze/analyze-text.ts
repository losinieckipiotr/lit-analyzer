...
export function analyzeText(
  ...
  const files: IVirtualSourceFile[] = (
  ...
  const sourceFilesToAnalyze = arrayDefined(
    files
      ...
      .map(file => program.getSourceFile(file.fileName))
  );
  const results = sourceFilesToAnalyze.map(sf =>
    analyzeSourceFile(sf, { program, ...config })
  );
  ...
  return {
    ...
    program,
    results,
    analyzedSourceFiles: sourceFilesToAnalyze
  };
}

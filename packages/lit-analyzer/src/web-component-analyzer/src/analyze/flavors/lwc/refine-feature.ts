...
function isLWCComponent(
  component: ComponentFeatureBase,
  context: AnalyzerVisitContext
) {
...
export const refineFeature: AnalyzerFlavor["refineFeature"] = {
  member: (
    ...
    if (isLWCComponent(member, context)) {
      const visibility = hasApiDecorator(member.node, context)
        ? "public"
        : "protected";
      return {
        ...member,
  ...
  method: (
    ...
    if (isLWCComponent(method, context)) {
      const visibility = hasApiDecorator(method.node, context)
        ? "public"
        : "protected";
      return {
        ...method,

...
import { ComponentMethod } from "../../../lib/analyze/wca-types.js";
 ...
 */
export function getTypeHintFromMethod(
  ...
  for (const parameterJsDocTag of method.jsDoc?.tags?.filter(
    tag => tag.tag === "param"
  ) || []) {
    const parsed = parameterJsDocTag.parsed();
    if (parsed.type != null) {
      signature = signature.replace(
        new RegExp(`${parsed.name}(.*?:\\s*)any\\[?]?`),
        `${parsed.name}$1${parsed.type}`
      );
    }
  }

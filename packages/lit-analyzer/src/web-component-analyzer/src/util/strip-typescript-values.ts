 ...
 */
export function stripTypescriptValues(
  input: unknown,
  checker: TypeChecker,
  seenValues = new Set<any>()
): unknown {
  ...
  if (input == null) {
    ...
    const title =
      "escapedText" in input ? (input as any).escapedText : undefined;
    ...
    for (const [key, value] of Object.entries(input)) {
      ...
      if (
        ...
        (!Array.isArray(strippedValue) || strippedValue.length > 0)
      ) {

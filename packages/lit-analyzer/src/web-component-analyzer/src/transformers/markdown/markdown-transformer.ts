    ...
    definitions.map(definition => {
      ...
      const mixins = arrayDedupe(
        arrayDefined(
          getMixinHeritageClausesInChain(declaration).map(
            clause =>
              clause.declaration?.symbol?.name ||
              (clause.identifier.getText() as string | undefined)
          )
      ...
      const attributes = filterVisibility(
        ...
        declaration.members.filter(
          (m): m is ComponentMemberAttribute => m.kind === "attribute"
        )
      ...
      const events = declaration.events.sort((a, b) =>
        a.name < b.name ? -1 : 1
      );
      const cssProps = declaration.cssProperties.sort((a, b) =>
        a.name < b.name ? -1 : 1
      );
      const cssParts = declaration.cssParts.sort((a, b) =>
        a.name < b.name ? -1 : 1
      );
...
function cssPropSection(
  ...
  return (
    markdownHeader("CSS Custom Properties", 2, config) +
    "\n" +
    markdownTable(rows)
  );
}
...
function cssPartSection(
  ...
  rows.push(
    ...cssPart.map(part => [
      (part.name && markdownHighlight(part.name)) || "",
      part.jsDoc?.description || ""
    ])
  );
  return (
    markdownHeader("CSS Shadow Parts", 2, config) + "\n" + markdownTable(rows)
  );
}
...
function eventSection(
  ...
  rows.push(
    ...events.map(event => [
      ...
      markdownHighlight(
        getTypeHintFromType(event.typeHint ?? event.type?.(), checker, config)
      ),
...
function slotSection(
  ...
  rows.push(
    ...slots.map(slot => [
      ...
      (slot.permittedTagNames &&
        slot.permittedTagNames
          .map(tagName => markdownHighlight(tagName))
          .join(" | ")) ||
        "",
...
function memberAttributeSection(
  ...
  const rows: string[][] = [
    ["Attribute", "Visibility", "Type", "Default", "Description"]
  ];

  // Add members as rows one by one
  for (const member of members) {
    const attrName = markdownHighlight(member.attrName);
    const type = markdownHighlight(
      getTypeHintFromType(member.typeHint ?? member.type?.(), checker, config)
    );
    const visibility = member.visibility || "public";
    const def =
      (member.default !== undefined ? JSON.stringify(member.default) : "") ||
...
function memberPropertySection(
  ...
  for (const member of members) {
    ...
    const attrName =
      (member.attrName && markdownHighlight(member.attrName)) || "";
    const visibility = member.visibility || "public";
    const type = markdownHighlight(
      getTypeHintFromType(member.typeHint ?? member.type?.(), checker, config)
    );
    const mods =
      member.modifiers != null ? Array.from(member.modifiers).join(", ") : "";
...
function shouldShowVisibility<T extends { visibility?: VisibilityKind }>(
  ...
  return (
    ...
    items.some(
      method => method.visibility != null && method.visibility !== "public"
    )
  );
}

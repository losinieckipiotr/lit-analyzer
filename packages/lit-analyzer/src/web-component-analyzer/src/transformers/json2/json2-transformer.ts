...
export const json2Transformer: TransformerFunction = (
  ...
  const modules = flattenedAnalyzerResults.map(result =>
    analyzerResultToModuleDoc(result, context)
  );
 ...
 * @param context
 */
function analyzerResultToModuleDoc(
 ...
 * @param context
 */
function getExportsDocsFromAnalyzerResult(
  result: AnalyzerResult,
  context: TransformerContext
): ExportDoc[] {
 ...
 * @param context
 */
function getFunctionDocsFromAnalyzerResult(
  result: AnalyzerResult,
  context: TransformerContext
): FunctionDoc[] {
 ...
 * @param context
 */
function getVariableDocsFromAnalyzerResult(
  result: AnalyzerResult,
  context: TransformerContext
): VariableDoc[] {
...
function getExportsDocFromDeclaration(
  ...
  const superclassRef =
    superclassHeritage == null
      ? undefined
      : getReferenceFromHeritageClause(superclassHeritage, context);

  // Get all mixins
  const mixinHeritage = getMixinHeritageClauses(declaration);
  const mixinRefs = arrayDefined(
    mixinHeritage.map(h => getReferenceFromHeritageClause(h, context))
  );
  ...
  if (definition != null) {
    ...
    const cssProperties = getCSSPropertyDocsFromDeclaration(
      declaration,
      context
    );
...
function getEventDocsFromDeclaration(
  ...
  return filterVisibility(context.config.visibility, declaration.events).map(
    event => {
      ...
      const typeName =
        simpleType.kind === "GENERIC_ARGUMENTS"
          ? simpleType.target.name
          : simpleType.name;
      const customEventDetailType =
        typeName === "CustomEvent" && simpleType.kind === "GENERIC_ARGUMENTS"
          ? simpleType.typeArguments[0]
          : undefined;
      ...
      return {
        ...
        type:
          typeName == null || simpleType.kind === "ANY" ? "Event" : typeName,
        detailType:
 ...
 * @param context
 */
function getSlotDocsFromDeclaration(
  declaration: ComponentDeclaration,
  context: TransformerContext
): SlotDoc[] {
...
function getCSSPropertyDocsFromDeclaration(
  ...
  return declaration.cssProperties.map(cssProperty => ({
    ...
    default:
      cssProperty.default != null
        ? JSON.stringify(cssProperty.default)
        : undefined,
 ...
 * @param context
 */
function getCSSPartDocsFromDeclaration(
  declaration: ComponentDeclaration,
  context: TransformerContext
): CSSPartDoc[] {
...
function getAttributeDocsFromDeclaration(
  ...
  for (const member of filterVisibility(
    context.config.visibility,
    declaration.members
  )) {
...
function getClassMemberDocsFromDeclaration(
  ...
  return [
    ...getFieldDocsFromDeclaration(declaration, context),
    ...getMethodDocsFromDeclaration(declaration, context)
  ];
}
 ...
 * @param context
 */
function getMethodDocsFromDeclaration(
  declaration: ComponentDeclaration,
  context: TransformerContext
): MethodDoc[] {
  const methodDocs: MethodDoc[] = [];

  for (const method of filterVisibility(
    context.config.visibility,
    declaration.methods
  )) {
    ...
    if (node !== undefined && context.ts.isMethodDeclaration(node)) {
      ...
      for (const param of node.parameters) {
        ...
        const { description, typeHint } = getParameterFromJsDoc(
          name,
          method.jsDoc
        );
        ...
        parameters.push({
          ...
          type: getTypeHintFromType(
            typeHint ||
              (param.type != null
                ? context.checker.getTypeAtLocation(param.type)
                : undefined),
            context.checker,
            context.config
          ),
    ...
    const { description: returnDescription, typeHint: returnTypeHint } =
      getReturnFromJsDoc(method.jsDoc);
 ...
 * @param context
 */
function getFieldDocsFromDeclaration(
  declaration: ComponentDeclaration,
  context: TransformerContext
): FieldDoc[] {
...
function getReferenceForNode(
  ...
  const isLib =
    ...
    sourceFile.fileName.match(/typescript\/lib.*\.d\.ts$/) != null;
  if (isLib) {
 ...
 * @param jsDoc
 */
function getParameterFromJsDoc(
  name: string,
 ...
 * @param jsDoc
 */
function getReturnFromJsDoc(jsDoc: JsDoc | undefined): {
  description?: string;
  typeHint?: string;
} {
...
function flattenAnalyzerResults(results: AnalyzerResult[]): AnalyzerResult[] {
  ...
  return results.map(result => {
    ...
    return {
      ...result,
      declarations:
        declarations != null ? Array.from(declarations) : result.declarations
    };
  });
}

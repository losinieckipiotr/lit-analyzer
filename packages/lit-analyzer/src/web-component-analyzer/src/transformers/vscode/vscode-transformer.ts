...
export const vscodeTransformer: TransformerFunction = (
  ...
  const definitions = results
    .map(res => res.componentDefinitions)
...
function definitionToHtmlDataTag(
  ...
  const customElementAttributes = arrayDefined(
    declaration.members.map(d => componentMemberToVscodeAttr(d, checker))
  );
  const eventAttributes = arrayDefined(
    declaration.events.map(e => componentEventToVscodeAttr(e, checker))
  );
  ...
  return {
    ...
    description: formatMetadata(declaration.jsDoc, {
      Events: declaration.events.map(e =>
        formatEntryRow(e.name, e.jsDoc, e.type?.(), checker)
      ),
...
function componentEventToVscodeAttr(
  event: ComponentEvent,
  checker: TypeChecker
): HtmlDataAttr | undefined {
  return {
    name: `on${event.name}`,
    description: formatEntryRow(
      event.name,
      event.jsDoc,
      event.type?.(),
      checker
...
function componentMemberToVscodeAttr(
  ...
  return {
    ...
    description: formatMetadata(
      ...
      ),
      {
        ...
        Default:
          member.default === undefined ? undefined : String(member.default)
      }
    ),
...
function typeToVscodeValuePart(
  ...
  switch (simpleType.kind) {
    ...
    case "ENUM":
      return {
        values: typesToStringUnion(simpleType.types.map(({ type }) => type))
      };
    case "UNION":

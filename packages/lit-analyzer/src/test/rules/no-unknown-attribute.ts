...
tsTest(
  "Don't report unknown attributes when 'no-unknown-attribute' is turned off",
  t => {
    const { diagnostics } = getDiagnostics("html`<input foo='' />`", {
...
tsTest("Don't report unknown attributes on unknown element", t => {
  const { diagnostics } = getDiagnostics(
    "html`<unknown-element foo=''></unknown-element>`",
    {
...
tsTest("Don't report unknown data- attributes", t => {
  const { diagnostics } = getDiagnostics("html`<input data-foo='' />`", {
    rules: { "no-unknown-attribute": true }
  });
  hasNoDiagnostics(t, diagnostics);
});
...
tsTest("Don't report element expressions", t => {
  const { diagnostics } = getDiagnostics("html`<input ${x} />`", {
    rules: { "no-unknown-attribute": true }
  });
  hasNoDiagnostics(t, diagnostics);
});

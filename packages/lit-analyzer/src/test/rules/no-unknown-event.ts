...
tsTest(
	"Don't report unknown events when 'no-unknown-event' is turned off",
	t => {
		const { diagnostics } = getDiagnostics(
			"html`<input @foo='${console.log}' />`",
...
tsTest("Don't report unknown events on unknown element", t => {
	const { diagnostics } = getDiagnostics(
		"html`<unknown-element @foo='${console.log}'></unknown-element>`",
		{

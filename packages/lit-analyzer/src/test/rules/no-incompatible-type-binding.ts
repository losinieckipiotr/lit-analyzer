...
tsTest(
  "Attribute binding: 'no-incompatible-type-binding' is not emitted when the rule is turned off",
  (t) => {
...
tsTest(
  "Attribute binding: String literal (a number) is assignable to number",
  (t) => {
...
tsTest(
  "Attribute binding: String literal (not a number) is not assignable to number",
  (t) => {
    const { diagnostics } = getDiagnostics('html`<input maxlength="foo" />`');
    hasDiagnostic(t, diagnostics, "no-incompatible-type-binding");
  },
...
tsTest(
  "Attribute binding: Number type expression is assignable to number",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input maxlength="${123}" />`',
    );
    hasNoDiagnostics(t, diagnostics);
  },
);
...
tsTest(
  "Attribute binding: String literal type expression (a number) is assignable to number",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input maxlength="${"123"}" />`',
    );
...
tsTest(
  "Attribute binding: String literal type expression (not a number) is not assignable to number",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input maxlength="${"foo"}" />`',
    );
    hasDiagnostic(t, diagnostics, "no-incompatible-type-binding");
  },
);
...
tsTest(
  "Attribute binding: String type expression is not assignable to number",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input maxlength="${{} as string}" />`',
    );
    hasDiagnostic(t, diagnostics, "no-incompatible-type-binding");
  },
);
...
tsTest(
  ...
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input maxlength="${{} as "123" | "321"}" />`',
    );
    hasNoDiagnostics(t, diagnostics);
  },
);
...
tsTest(
  ...
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input maxlength="${{} as "123" | "foo"}" />`',
    );
    hasDiagnostic(t, diagnostics, "no-incompatible-type-binding");
  },
);
...
tsTest(
  "Attribute binding: String literal (a number) is assignable to string",
  (t) => {
...
tsTest(
  "Attribute binding: String literal expression is assignable to string",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input placeholder="${"foo"}" />`',
    );
    hasNoDiagnostics(t, diagnostics);
  },
);
...
tsTest(
  "Attribute binding: Number type expression is assignable to string",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input placeholder="${123}" />`',
    );
    hasNoDiagnostics(t, diagnostics);
  },
);
...
tsTest(
  "Attribute binding: String literal (0 length) is assignable to number",
  (t) => {
...
tsTest(
  "Attribute binding: String literal (0 length) is assignable to string",
  (t) => {
...
tsTest(
  "Attribute binding: String literal (0 length) is assignable to boolean",
  (t) => {
...
tsTest(
  "Attribute binding: String literal is not assignable to boolean",
  (t) => {
...
tsTest(
  "Attribute binding: Number type expression is not assignable to boolean",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input required="${123}" />`',
      { rules: { "no-boolean-in-attribute-binding": false } },
    );
    hasDiagnostic(t, diagnostics, "no-incompatible-type-binding");
  },
...
tsTest(
  "Attribute binding: Boolean type expression (true) is assignable to 'true'|'false'",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input aria-expanded="${true}" />`',
      { rules: { "no-boolean-in-attribute-binding": false } },
    );
    hasNoDiagnostics(t, diagnostics);
  },
...
tsTest(
  "Attribute binding: Boolean type expression (false) is assignable to 'true'|'false'",
  (t) => {
    const { diagnostics } = getDiagnostics(
      'html`<input aria-expanded="${false}" />`',
      { rules: { "no-boolean-in-attribute-binding": false } },
    );
    hasNoDiagnostics(t, diagnostics);
  },
...
tsTest(
  "Boolean binding: Boolean is assignable in a boolean attribute binding",
  (t) => {
    const { diagnostics } = getDiagnostics(
...
tsTest(
  "Boolean binding: String is not assignable in boolean attribute binding",
  (t) => {
    const { diagnostics } = getDiagnostics(
...
tsTest(
  "Property binding: String literal type expression is not assignable to boolean property",
  (t) => {
    const { diagnostics } = getDiagnostics([
...
tsTest(
  "Property binding: String literal (0 length) type expression is not assignable to boolean property",
  (t) => {
    const { diagnostics } = getDiagnostics([
      makeElement({ properties: ["required = false"] }),
...
tsTest(
  "Property binding: Number type expression is not assignable to boolean property",
  (t) => {
    const { diagnostics } = getDiagnostics([
      makeElement({ properties: ["required = false"] }),
...
tsTest(
  "Property binding: Boolean type expression is not assignable to boolean property",
  (t) => {
    const { diagnostics } = getDiagnostics([
      makeElement({ properties: ["required = false"] }),
...
tsTest(
  "Attribute binding: 'ifDefined' directive correctly removes 'undefined' from the type union 1",
  (t) => {
    const { diagnostics } = getDiagnostics(
...
tsTest(
  "Attribute binding: 'ifDefined' directive correctly removes 'undefined' from the type union 2",
  (t) => {
    const { diagnostics } = getDiagnostics(
...
tsTest(
  "Attribute binding: the role attribute is correctly type checked when given valid items",
  (t) => {
    const { diagnostics } =
      getDiagnostics(`html\`<div role="button listitem"></div>\`
	`);
...
tsTest(
  "Attribute binding: the role attribute is correctly type checked when given invalid items",
  (t) => {
    const { diagnostics } =
...
tsTest(
  "Attribute binding: the target attribute is correctly type checked when given a string",
  (t) => {
    const { diagnostics } =
      getDiagnostics(`html\`<a target="custom-target"></a>\`
	`);

	...
	absorbCollection(collection: Partial<HtmlDataCollection>): void {
		if (collection.tags != null) {
			...
			collection.tags.forEach(tag =>
				this._globalTags.set(tag.tagName.toLowerCase(), tag)
			);
		}

		if (collection.global?.attributes != null) {
			// For now, lowercase all names because "parse5" doesn't distinguish when parsing
			collection.global.attributes.forEach(attr =>
				this._globalAttributes.set(attr.name.toLowerCase(), attr)
			);
		}

		if (collection.global?.events != null) {
			// For now, lowercase all names because "parse5" doesn't distinguish when parsing
			collection.global.events.forEach(evt =>
				this._globalEvents.set(evt.name.toLowerCase(), evt)
			);
		}

		if (collection.global?.properties != null) {
			// For now, lowercase all names because "parse5" doesn't distinguish when parsing
			collection.global.properties.forEach(prop =>
				this._globalProperties.set(prop.name.toLowerCase(), prop)
			);
		}

		if (collection.global?.slots != null) {
			// For now, lowercase all names because "parse5" doesn't distinguish when parsing
			collection.global.slots.forEach(slot =>
				this._globalSlots.set(slot.name.toLowerCase(), slot)
			);
		}

		if (collection.global?.cssParts != null) {
			// For now, lowercase all names because "parse5" doesn't distinguish when parsing
			collection.global.cssParts.forEach(cssPart =>
				this._globalCssParts.set(cssPart.name.toLowerCase(), cssPart)
			);
		}
		...
		if (collection.global?.cssProperties != null) {
			...
			collection.global.cssProperties.forEach(cssProperty =>
				this._globalCssProperties.set(
					cssProperty.name.toLowerCase(),
					cssProperty
				)
			);
		}
	...
	forgetCollection({
		...
		if (events != null)
			events.forEach(eventName =>
				this._globalEvents.delete(eventName.toLowerCase())
			);
		if (attributes != null)
			attributes.forEach(attrName =>
				this._globalAttributes.delete(attrName.toLowerCase())
			);
		if (properties != null)
			properties.forEach(propName =>
				this._globalProperties.delete(propName.toLowerCase())
			);
		if (slots != null)
			slots.forEach(slotName =>
				this._globalSlots.delete(slotName.toLowerCase())
			);
		if (cssParts != null)
			cssParts.forEach(partName =>
				this._globalCssParts.delete(partName.toLowerCase())
			);
		if (cssProperties != null)
			cssProperties.forEach(cssPropName =>
				this._globalCssProperties.delete(cssPropName.toLowerCase())
			);
	}
